export default {
  ...
  knowledgeEditor: {
    ...
    sidebar: {
      ...
      asr: "音频处理",
      datasource: "数据源",
      imageCompression: "图片存储优化",
      share: "共享管理",
      indexing: "索引策略",
    },
    imageCompression: {
      title: "存量图片存储优化",
      description: "将这个知识库中已完成解析的大图片压缩后替换到对象存储，降低长期存储与检索流量。",
      scopeNote: "只处理当前知识库的已有图片，不重新执行 OCR、多模态识别或 RAG 索引。系统会先写入压缩图并切换记录，再删除原图。",
      eligible: "待处理图片",
      sourceSize: "当前总体积",
      target: "单图目标",
      progress: "处理进度",
      ...
      running: "正在优化",
      started: "图片优化任务已开始",
      previewFailed: "无法获取图片优化预估",
      startFailed: "图片优化任务启动失败",
      confirmTitle: "开始压缩存量图片？",
      confirmBody: "将处理 {count} 张图片，当前总体积约 {size}。该任务在后台低优先级运行，失败项会分轮重试，RAG 索引不会重建。",
    },
    errors: {
  ...
  datasource: {
    ...
    resourceRequired: "请选择要同步的空间、知识库或目录；如果列表权限不足，也可以返回上一步手动填写 ID。",
    untitled: "无标题",
    ...
    dingtalk: {
      ...
      drive: "云盘",
      wiki: "知识库",
      unionIdHint: "填写拥有对应云盘或知识库访问权限的钉钉成员 UnionID，不是企业 CorpId。",
      manualResourceIds: "无法从列表选择时，手动填写 ID（可选）",
      manualResourceIdsHint: "优先使用下一步的列表选择。只有列表权限暂时不足时才手动填写，每行一个；知识库填写 workspaceId，云盘填写 spaceId。",
      manualModeHint: "已启用手动 ID 模式，下一步会跳过列表选择，并只保存这里填写的 ID。",
      manualIdUrlWarning: "请填写 ID，不要填写浏览器地址。知识库请填 workspaceId，云盘请填 spaceId。",
      manualIdTypeMismatchWiki: "当前选择的是知识库，请填写知识库 ID（wiki:<ID> 或直接填写 workspaceId），不要填写 space:<ID>。",
      manualIdTypeMismatchDrive: "当前选择的是云盘，请填写云盘 ID（space:<ID> 或直接填写 spaceId），不要填写 wiki:<ID>。",
    },
    comingSoon: "即将支持",
    docHint: "在以下地址获取凭证：",
    openDoc: "打开文档",
